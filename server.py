...
send = lambda data: TCP_CONN.send(bytes(data, "utf-8")) and time.sleep(0.5)
...
def recvall(BUFFER):
    data = b""
    while (len(data) < int(BUFFER)):
        data += recv(int(BUFFER))
...
def ChangeWallpaper():
    ...
    TCP_CONN.send(bytes(str(len(fileContent)), "utf-8")); time.sleep(0.5)
